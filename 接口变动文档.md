...
### 托管媒体在 `hybrid / object_storage` 下补齐真实 COS 写入与对象存储 metadata
...
- 接口：
  ...
  - 以及其它复用统一托管上传持久化逻辑的媒体导入、上传与资产回显接口
- 变动性质：未新增路由；调整的是既有托管媒体在 `MEDIA_STORAGE_MODE=hybrid/object_storage` 下的后端持久化语义，并补齐已有 `metadata_json` 中对象存储字段的真实回填。

本轮已稳定以下承接行为：

- 当后端通过统一托管上传逻辑写入媒体时：
  - `persist_uploaded_file(...)` 与 `persist_remote_file(...)` 不再只写本地 `/uploads/...`
  - 在 `hybrid / object_storage` 模式下，还会继续把同一份字节内容同步 `put_object` 到腾讯云 COS
- 对于创作上传、手动资产导入和派生媒体 playback metadata：
  - `metadata_json` 现在会继续沿用既有 `file_url / preview_url / download_url` 顶层消费方式
  - 同时补齐已有预留但此前经常缺失的对象存储字段：
    ...
    - `cdn_url`
- 资产列表、详情与创作上传接口：
  - 顶层路由和主要响应结构不需要前端新增字段适配
  - 若前端或脚本侧会读取 `metadata_json`，现在可以稳定拿到对象存储定位字段，用于后续预览、下载或排障

说明：

- 本轮针对的是“线上对象存储配置已经存在，但新生成数据没有真正写进 COS”的问题。
- 变化点不是新增一组新接口，而是让既有上传与资产读取接口的运行时行为从“仅本地托管”升级为“本地托管 + COS 同步写入”，并把对象存储字段真实回填到响应中的 `metadata_json`。

## 2026-06-23

### 分镜视频上传接口修复 `video_asset` 未定义导致的 500
...
- 变动性质：未新增路由、未新增请求字段、未新增响应字段；修复的是既有分镜视频上传接口在“视频已落盘、资产尚未创建”阶段错误引用未定义变量而触发 `500` 的后端运行时语义。

本轮已稳定以下承接行为：

- 当分镜视频通过 `upload-video` 成功落盘、但对应 `Asset` 记录尚未创建时：
  - 后端不再提前引用未定义的 `video_asset`
  - 会改用当前请求上下文 `user_id / project_id` 继续组装媒体字段
- 上传成功后：
  - 分镜仍会继续写回既有 `video_url / poster_url / preview_video_url / download_url / available_qualities / hls_url`
  - 接口仍返回既有 `StoryboardResponse`，前端不需要新增字段适配
- 分镜视频资产创建完成后：
  - 列表、详情与回显继续按既有资产语义消费，不改变现有页面展示字段

说明：

- 本轮针对的是你线上已出现的 `POST /upload-video -> 500 Internal Server Error`。
- 对外接口契约未变化，变化点仅在后端上传成功路径的运行时收口逻辑。

### 分镜视频任务补齐“已落盘但误判失败”的终态收口
...
- 接口：
  ...
  - `GET /api/projects/{project_id}/storyboards`
  - `GET /api/projects/{project_id}/storyboards/{storyboard_id}`
- 变动性质：未新增路由、未新增请求字段、未新增响应字段；修复的是既有分镜视频任务在落盘成功后被错误打成 `failed` 的后端运行时语义。

本轮已稳定以下承接行为：

- 当分镜视频文件已经托管成功，但对应资产记录尚未创建时：
  - 后端不再错误引用未定义的 `video_asset`
  - 任务会继续使用当前 `user_id / project_id` 上下文组装媒体字段并完成后续落库
- `sb_video` 任务在“文件已落盘、分镜与资产已写回”的正常路径下：
  - 会继续收口到 `completed`
  - `results` 中继续返回既有 `video_url / preview_video_url / poster_url / download_url / preview_ready` 等字段
- 前端继续按既有任务轮询接口消费即可，不需要新增字段适配；已有旧视频的镜头重新生成后，也应以最新结果覆盖当前分镜视频回显

说明：

- 本轮修复针对的是“服务器目录里已经出现视频文件，但任务状态仍是 `failed`、前端不回显”的错位现象。
- 对外接口契约和字段名未变化，变化点仅在后端任务终态收口语义与前端结果覆盖策略。

### OneLinkAI 恢复 chat 预置，图片/视频继续保持收口

- 接口：
  - `GET /api/models`
  - `POST /api/providers/oneclick-setup`
  - `POST /api/providers/oneclick-cleanup`
- 变动性质：未新增路由、未新增字段；调整的是 OneLinkAI 内置预置模型白名单及其同步/清理后的返回结果语义。

本轮已稳定以下承接行为：

- OneLinkAI 当前已恢复 `chat` 类预置模型入口，执行 `oneclick-setup` 或 `oneclick-cleanup` 后会重新同步聊天模型集合
- 图片与视频入口继续保持收口：
  - 图片仅保留 `doubao-seedream-5.0-lite`
  - 视频仅保留 `doubao-seedance-2.0`
  - 视频仅保留 `doubao-seedance-2-0-fast`
- 原先仍未恢复的聚合图片模型、Kling 图像/视频模型等入口，当前继续视为历史旧模型：
  - 新执行 `oneclick-setup` 或 `oneclick-cleanup` 后，会按现有清理逻辑移除
- 默认模型口径保持不变：
  - chat 默认仍为 `gpt-4o`
  - 图片默认仍为 `doubao-seedream-5.0-lite`
  - 视频默认仍为 `doubao-seedance-2.0`

说明：

- 本轮目标是重新打开 OneLinkAI 的 chat 模型，同时继续保持图片/视频侧的收口策略不变。
- 前端现有模型选择交互、字段结构与路由不需要调整；变化点主要在 OneLinkAI 模型清单本身与旧模型清理结果。

### 模型运行时凭证解析在重复 `model_id` 场景下优先收口到官方 provider
...
- 接口：
  ...
  - 以及其它复用统一模型运行时选择逻辑的图片/视频生成接口
- 变动性质：未新增路由、未新增请求字段、未新增响应字段；调整的是后端在“同一用户、不同服务商下存在相同 `model_id`”时的运行时凭证选择语义。

本轮已稳定以下承接行为：

- 当同一 `model_id` 同时存在于多个启用中的 provider 下时：
  - 后端会先按统一排序口径选择候选 provider，排序与模型列表展示口径保持一致
  - 对 `doubao-*` 这类已支持官方 `volcengine` provider 的模型，会优先命中官方 `volcengine` 凭证，而不是历史遗留的聚合 `onelink` 凭证
- 对不属于官方优先规则的普通重复模型：
  - 继续沿用既有“默认模型优先，其次较新的 provider 配置优先”的承接顺序
- 本轮修复后，分镜图、创作图片及其它复用同一运行时凭证解析逻辑的链路，在重复 `model_id` 场景下不再容易因为拿错旧 provider API Key 而触发上游 `401 apikey 验证失败`

说明：

- 本轮针对的是“官方 provider 与聚合 provider 共用同一模型 ID”导致的运行时错配问题，目标是让用户在不改前端现有模型选择交互的前提下，后端也能稳定取到正确凭证。
- 本轮不改变前端现有请求方式，也不要求页面新增 provider 维度的显式传参；变化点仅在后端统一模型运行时解析逻辑。
